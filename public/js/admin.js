(() => {
  ...
  function extractMacroFields(root) {
    ...
    for (const node of nodes) {
      ...
      const text = textFrom(node, "[data-text], .macro-body, pre, p");
  ...
  function parseOverrideHtml(basePane, html) {
    ...
    if (root) {
      ...
      pane.format = format;
      if (format === "dual") {
        pane.english = textFrom(root, '[data-language="english"] [data-content]');
        pane.arabic = textFrom(root, '[data-language="arabic"] [data-content]');
      } else {
    ...
    if (basePane.format === "support_macro") {
      ...
      if (englishFields.length || arabicFields.length) {
        ...
        pane.english = { title: pane.english?.title || pane.title, fields: [{ label: "Text", text: all.textContent.trim() }] };
      }
    } else {
      const rtl = all.querySelector('[dir="rtl"], .lang-block.ar, .ar');
      const ltr = all.querySelector('[dir="ltr"], .lang-block.en, .en');
      const wholeText = cleanText(all.textContent || "").trim();
      pane.english = cleanText(ltr?.textContent || (!rtl ? wholeText : pane.english || ""));
      pane.arabic = cleanText(rtl?.textContent || pane.arabic || "");
    }
    pane._adminOverride = true;
    pane._overrideHtml = html;
    return pane;
