(function(){
  ...
  function paneTargetForTopicEdit(pane){
    ...
    if(existing && !existing.classList.contains('sugo-admin-current-topic-toolbar')) return existing;
    var preferred=pane.querySelector(':scope > .doc-card, :scope > .ai-answer-card, :scope > .macro-grid, :scope > .lang-block');
    ...
    return kids.find(function(el){
      return el && el.classList &&
        ...
        !el.classList.contains('sugo-admin-current-topic-toolbar') &&
        !el.classList.contains('sugo-visible-favorite-toolbar') &&
        !el.classList.contains('sugo-fr-cardbar') &&
        !el.hasAttribute('data-sugo-current-topic-edit');
    }) || null;
  }
  ...
  function paneHtmlForTopicEdit(paneId){
    ...
    var targetHtml = target ? String(target.innerHTML || '') : '';
    // Do not read the admin toolbar as topic content. If the extracted HTML has no real topic markers, use the paneContent store.
    if(targetHtml && /(?:doc-card|macro-col|sugo-section|lang-divider|macro-body|copy-buttons)/.test(targetHtml)) return targetHtml;
    var store=getPaneStoreForTopicEdit();
    var rec=store && store[paneId];
    if(!rec) return targetHtml || '';
  ...
  function cloneTextForTopicEdit(node){
    ...
    clone.querySelectorAll('button,.copy-buttons,.sugo-view-controls,.sugo-min-filter-panel,.sugo-efficiency-panel,.sugo-filter-empty,.lang-divider,.sugo-internal-title,.close-pane-btn,.sugo-direct-edit-toolbar,.sugo-inline-edit-toolbar,.sugo-admin-current-topic-toolbar,.sugo-admin-current-topic-edit-btn,.sugo-visible-favorite-toolbar,.sugo-fr-cardbar').forEach(function(el){ el.remove(); });
  ...
  function topicMetaHasRealContent(meta){
    if(!meta || typeof meta !== 'object') return false;
    return ['enText','arText','enAnswer','arAnswer','enTicket','arTicket','enMention','arMention','enForm','arForm'].some(function(key){
      return !!cleanText(meta[key] || '', 300000);
    });
  }
  ...
  function topicMetaFromPane(paneId, fallbackName){
    ...
    // Important: the saved/integrated menu can contain an empty meta shell for built-in topics.
    // Do not trust that empty shell, otherwise the edit form opens with blank Answer/Ticket fields.
    // Only use saved meta when it actually contains content; otherwise read the currently rendered pane.
    if(meta && topicMetaHasRealContent(meta)) return meta;
    ...
    if(enTextNode || arTextNode){
      ...
      var enTitle=wrap.querySelector('.macro-col[dir="ltr"] .macro-title, .sugo-section[data-lang="en"] .macro-title, .sugo-section[data-lang="en"] h1, .sugo-section[data-lang="en"] h2');
      var arTitle=wrap.querySelector('.macro-col[dir="rtl"] .macro-title, .sugo-section[data-lang="ar"] .macro-title, .sugo-section[data-lang="ar"] h1, .sugo-section[data-lang="ar"] h2');
