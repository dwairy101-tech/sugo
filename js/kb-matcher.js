(() => {
  ...
  const EXTRA_ACCURACY_ROUTES = Object.freeze([
    { name:"country-change-follow-up", all:[["country"],["change"],["follow up","follow-up","متابعه","متابعة","قدمت طلب","تم تقديم"]], any:[], ticketTopicIds:["sv-tickets-country-2"], topicIds:["sv-refined-change-country-follow-up"] },
    { name:"country-change-direct", all:[["country"],["change"]], any:[], ticketTopicIds:["sv-tickets-country-1"], topicIds:["sv-refined-change-country"] },
    { name:"game-crash-issue", all:[["game"],["crash"]], any:[], ticketTopicIds:[], topicIds:["function-games-crashing"] },
    { name:"app-crash-direct", all:[["crash"]], any:[["app","application","التطبيق"]], ticketTopicIds:[], topicIds:["sv-refined-app-crash-refresh-steps"] },
    { name:"password-reset-submitted", all:[["password"],["submitted","تم تقديم","تم رفع","متابعه","متابعة","follow up","follow-up"]], any:[], ticketTopicIds:["sv-tickets-binding-request-reset-password"], topicIds:["sv-refined-password-reset-request-submitted"] },
    { name:"password-reset-guide", all:[["password"]], any:[["نسيت","forgot","reset","استرجاع","اعاده تعيين","إعادة تعيين"]], ticketTopicIds:[], topicIds:["account-security-reset"] },
    { name:"coins-not-received-safe", all:[["recharge"],["notReceived"]], any:[], ticketTopicIds:["sv-tickets-coins-not-received"], topicIds:["payment-recharge-missing-coins"] },
    { name:"agency-create", all:[["agency"],["create"]], any:[], ticketTopicIds:["sv-tickets-agency-create"], topicIds:["sv-refined-create-host-agency"] },
    { name:"sexual-picture-in-messages", all:[["sexual"],["picture"],["message"]], any:[], ticketTopicIds:["sv-tickets-ban-sexual-picture"], topicIds:["sv-refined-ban-sexual-content-in-messages"] },
    ...
    { name:"login-issue-general", all:[["login","دخول","لا يدخل","تسجيل دخول","افوت","ادخل"],["account"]], any:[], ticketTopicIds:[], topicIds:["account-login-issues"] },
  ...
  function stripArabicArticle(token) {
    ...
    return value.length > 3 && value.indexOf("ال") === 0 ? value.slice(2) : value;
  }
  ...
  function detectAccuracyRoutes(query) {
    ...
    const uniqueDetected = [...explicit, ...detected]
      .filter((route, index, rows) => rows.findIndex((item) => item.name === route.name) === index);
    const specificity = (route) => (route.all?.length || 0) * 10 + (route.any?.length ? 1 : 0);
    const strongestSpecificity = uniqueDetected.reduce((best, route) => Math.max(best, specificity(route)), 0);
    return uniqueDetected
      .filter((route) => specificity(route) === strongestSpecificity)
      .map((route) => ({
        ...route,
  ...
  function ticketTitleQueryVariants(query, options = {}) {
    const original = normalize(query);
    if (!original || !isTicketMode(options)) return original ? [original] : [];
    const stripped = original
      .replace(/^(?:بدي|اريد|ارغب|اعمل|اعملي|انشئ|انشاء|سوي|سويلي)\s+(?:تذكره|تذكره دعم|تكت|ticket)\s+(?:عن|حول|بخصوص|لـ?|ل)\s+/u, "")
      .replace(/^(?:تذكره|تكت)\s+(?:عن|حول|بخصوص|لـ?|ل)\s+/u, "")
      .replace(/^(?:create|make|write|generate|open)\s+(?:a\s+)?(?:support\s+)?ticket\s+(?:about|for|regarding|on)\s+/i, "")
      .replace(/^(?:ticket)\s+(?:about|for|regarding|on)\s+/i, "")
      .trim();
    return [...new Set([original, stripped].filter(Boolean))];
  }

  function findExactTitleTopic(query, options = {}) {
    const wantedValues = ticketTitleQueryVariants(query, options);
    if (!wantedValues.length) return null;
    const ticketMode = isTicketMode(options);
    return buildTopics()
      .filter((topic) => topicAllowedForOutput(topic, options))
      .filter((topic) => wantedValues.some((wanted) =>
        topic.idNorm === wanted || (topic.titleAliasesNorm || []).includes(wanted)
      ))
      .sort((a, b) => {
        if (ticketMode && Boolean(a.isTicketMacro) !== Boolean(b.isTicketMacro)) return a.isTicketMacro ? -1 : 1;
  ...
  const GENERIC_TICKET_CONCEPTS = new Set(["ban", "unban", "reason", "account"]);

  function accuracyConcepts(value) {
    return Object.keys(ACCURACY_SYNONYMS).filter((key) => accHasAny(value, [key]));
  }

  function comparableToken(value) {
    return stripArabicArticle(normalize(value))
      .replace(/(ات|يات|ين|ون|ان|ه|ها|هم|ing|ed|es|s)$/u, "")
      .trim();
  }

  function relatedTicketToken(left, right) {
  ...
  function ticketTitleCandidateScore(topic, query, queryTokens, queryConcepts) {
    const aliases = topic.localizedTitles || [topic.title || ""];
    let best = null;
    aliases.forEach((alias) => {
      const aliasNormalized = normalize(alias);
      const titleTokens = tokenList(alias, false, SENTENCE_STOP);
      const matchedQueryTokens = queryTokens.filter((queryToken) =>
        titleTokens.some((titleToken) => relatedTicketToken(queryToken, titleToken))
      );
      const matchedTitleTokens = titleTokens.filter((titleToken) =>
        queryTokens.some((queryToken) => relatedTicketToken(queryToken, titleToken))
      );
      ...
      const titleConcepts = accuracyConcepts(alias);
      const matchedConcepts = queryConcepts.filter((concept) => titleConcepts.includes(concept));
      const conceptCoverage = queryConcepts.length ? matchedConcepts.length / queryConcepts.length : 0;
      const decisiveQueryConcepts = queryConcepts.filter((concept) => !GENERIC_TICKET_CONCEPTS.has(concept));
      const decisiveMatches = decisiveQueryConcepts.filter((concept) => titleConcepts.includes(concept));
      const missingDecisive = decisiveQueryConcepts.filter((concept) => !titleConcepts.includes(concept));
      const phraseHit = containsNormalized(aliasNormalized, normalize(query))
        || containsNormalized(normalize(query), aliasNormalized);
      let score = (queryCoverage * 125) + (titleCoverage * 45) + (conceptCoverage * 155);
      if (phraseHit && normalize(query).length >= 4) score += 80;
      score += decisiveMatches.length * 55;
      score -= missingDecisive.length * 90;
      if (topic.idNorm === normalize(query) || (topic.titleAliasesNorm || []).includes(normalize(query))) score += 1000;
      const row = {
        topic,
        alias,
        score: Math.round(Math.max(0, score) * 10) / 10,
        queryCoverage: Math.round(queryCoverage * 100) / 100,
        titleCoverage: Math.round(titleCoverage * 100) / 100,
        conceptCoverage: Math.round(conceptCoverage * 100) / 100,
        concepts: titleConcepts,
        matchedConcepts,
        decisiveMatches,
        missingDecisive
      };
  ...
  function resolveTicketTitleRoute(query, options = {}) {
    if (!isTicketMode(options)) return null;
    const normalizedQuery = normalize(query);
    if (!normalizedQuery) return null;
    const queryTokens = tokenList(query, false, SENTENCE_STOP);
    const queryConcepts = accuracyConcepts(query);
    const macros = buildTopics().filter((topic) => topic.isTicketMacro || String(topic.id || "").startsWith("sv-tickets-"));
    const ranked = macros
      .map((topic) => ticketTitleCandidateScore(topic, query, queryTokens, queryConcepts))
      .filter((row) => row && row.score > 0)
      .sort((a, b) =>
        b.score - a.score
        || b.queryCoverage - a.queryCoverage
        || b.conceptCoverage - a.conceptCoverage
    ...
    const strong = Boolean(
      exact
      || (best.score >= 245 && best.queryCoverage >= 0.58 && gap >= 14)
      || (best.score >= 285 && best.conceptCoverage >= 0.75 && gap >= 18)
    );
    const focusedAmbiguity = Boolean(
      !strong
      && second
      && best.score >= 185
      && second.score >= 170
      && gap <= 34
      && best.matchedConcepts.length >= 2
      && second.matchedConcepts.length >= 2
    );
    if (!strong && !focusedAmbiguity) return null;
    const candidates = focusedAmbiguity
      ? ranked.filter((row) => best.score - row.score <= 34).slice(0, 3)
      : [best];
    return {
      name: strong ? `ticket-title-${best.topic.id}` : "ticket-title-clarification",
      all: [],
      any: [],
      ticketTopicIds: candidates.map((row) => row.topic.id),
      topicIds: [],
      topicIdsFromRule: [],
      titleResolved: strong,
      titleAmbiguous: focusedAmbiguity,
      titleScore: best.score,
      titleGap: Math.round(gap * 10) / 10,
      titleCandidates: candidates.map((row) => ({
        id: row.topic.id,
        title: row.alias || row.topic.title,
        score: row.score,
  ...
  function precisionMatch(query, maxTopics, maxCharsPerTopic, preferredPaneId, options) {
    ...
    const ticketTitleRoute = exactTitleTopic
      ? {
          name: `ticket-title-${exactTitleTopic.id}`,
          all: [],
          any: [],
          ticketTopicIds: isTicketMode(resolvedOptions) ? [exactTitleTopic.id] : [],
          topicIds: isTicketMode(resolvedOptions) ? [] : [exactTitleTopic.id],
          topicIdsFromRule: [],
          titleResolved: true,
          titleAmbiguous: false,
          titleScore: 1200,
          titleGap: 1200,
          titleCandidates: [{ id: exactTitleTopic.id, title: exactTitleTopic.title, score: 1200, matchedConcepts: [] }]
        }
      : resolveTicketTitleRoute(query, resolvedOptions);
    const routes = exactTitleTopic
      ? [ticketTitleRoute]
      : (detectAccuracyRoutes(query).length ? detectAccuracyRoutes(query) : (ticketTitleRoute ? [ticketTitleRoute] : []));
    ...
    if (ticketTitleRoute?.titleAmbiguous) {
      confidence.confidence = "medium";
      confidence.label = "Medium";
      confidence.ambiguous = true;
      confidence.reason = "ticket-title-clarification";
    }
    ...
    return {
      ...
      ticketTitleMatch: ticketTitleRoute,
  ...
  function rebuildSentencePacket(result, topics, maxCharsPerTopic) {
    ...
    const promptEligibleTopics = topics.filter((topic) => {
      if (
        result?.outputType === "ticket"
        && topic.isTicketMacro
        && !topic.primary
        && !topic.selected
        && topic.id !== result.exactTitleTopicId
      ) return false;
      const secondaryRouteHit = result?.primaryRoute && (topic.hits || []).some((hit) =>
        (result.detectedRoutes || []).includes(hit) && hit !== result.primaryRoute.name
      );
      return !secondaryRouteHit || topic.primary || topic.selected || topic.id === result.exactTitleTopicId;
    });
    const contextTopics = promptEligibleTopics.slice(0, 6);
    const audit = contextTopics.map((topic, index) => `${index + 1}. ${topic.id} | title: ${topic.title || topic.label || ""} | score: ${Math.round((topic.score || 0) * 10) / 10} | hits: ${(topic.hits || []).slice(0, 8).join(", ") || "sentence-lexicon"}`).join("\n");
    ...
    result.topics = contextTopics;
    result.topicIds = contextTopics.map((topic) => topic.id);
    result.bestTopic = contextTopics[0] || null;
    const bestScore = contextTopics[0] ? Number(contextTopics[0].score || 0) : 0;
    const secondScore = contextTopics[1] ? Number(contextTopics[1].score || 0) : 0;
    const gap = contextTopics[1] ? bestScore - secondScore : bestScore;
    const routeCandidateIds = result.primaryRoute
      ? (result.outputType === "ticket"
          ? result.primaryRoute.ticketTopicIds || []
          : [...(result.primaryRoute.topicIds || []), ...(result.primaryRoute.topicIdsFromRule || [])])
          .filter((id) => contextTopics.some((topic) => topic.id === id))
      : [];
    const directResolved = Boolean(
      contextTopics[0]?.primary
      && (contextTopics[0]?.hits || []).some((hit) => String(hit).startsWith("direct:"))
      && gap >= 40
    );
    const routeResolved = Boolean(
      ...
      && contextTopics[0]?.primary
      && (
        (routeCandidateIds.length === 1 && routeCandidateIds[0] === contextTopics[0]?.id)
        || directResolved
      )
    );
    const routeAmbiguous = Boolean(
      result.primaryRoute
      && !result.exactTitleMatch
      && !directResolved
      && routeCandidateIds.length > 1
      && contextTopics.slice(0, 2).every((topic) => topic?.primary)
    );
    const exactResolved = Boolean(result.exactTitleMatch && contextTopics[0]?.id === result.exactTitleTopicId);
    ...
    if (routeResolved || exactResolved) finalConfidence = "high";
    ...
    result.hasMeaningfulMatch = Boolean(
      contextTopics.length
      && (
        exactResolved
        || routeResolved
        || (!finalAmbiguous && bestScore >= 42 && (gap >= 8 || contextTopics.length === 1))
      )
    );
    result.sentenceLexiconEnabled = true;
    result.retrievalVersion = VERSION;
    result.routeCandidateIds = routeCandidateIds;
    return result;
  }

  function match(query, maxTopics = 8, maxCharsPerTopic = 1400, preferredPaneId = null, options = {}) {
    const resolvedOptions = options && typeof options === "object" ? options : {};
    const topicCap = resolvedOptions.completeAnswer ? 6 : 5;
    const requestedTopics = Number.isFinite(Number(maxTopics)) && Number(maxTopics) > 0
      ? Number(maxTopics)
      : topicCap;
    const resolvedMaxTopics = Math.max(1, Math.min(topicCap, requestedTopics));
