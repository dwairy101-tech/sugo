(() => {
  ...
  function getOutputLanguage(value) {
    return OUTPUT_LANGUAGES.find((item) => item.value === value) || OUTPUT_LANGUAGES[0];
  }
  ...
  function createTicketDetailsForm() {
    ...
    language.className = "ticket-language-field";
    language.innerHTML = `
      <span class="ticket-language-field__label" id="ticketLanguageLabel">Output language</span>
      <div class="ticket-language-control" role="group" aria-labelledby="ticketLanguageLabel"></div>
    `;
    const languageControl = language.querySelector(".ticket-language-control");
    OUTPUT_LANGUAGES.forEach((item) => {
      const button = document.createElement("button");
      button.type = "button";
      button.className = "ticket-language-button";
      button.dataset.ticketLanguage = item.value;
      button.textContent = item.label;
      const selected = ticketWorkspaceState.language === item.value;
      button.classList.toggle("is-selected", selected);
      button.setAttribute("aria-pressed", String(selected));
      button.addEventListener("click", () => {
        ticketWorkspaceState.language = getOutputLanguage(item.value).value;
        languageControl.querySelectorAll("[data-ticket-language]").forEach((candidate) => {
          const isSelected = candidate.dataset.ticketLanguage === ticketWorkspaceState.language;
          candidate.classList.toggle("is-selected", isSelected);
          candidate.setAttribute("aria-pressed", String(isSelected));
        });
        emitTicketDetailsChange("language");
      });
  ...
  function renderTicketPreviewPanel() {
    ...
    if (snapshot.generatedOutput) {
      ...
      generated.dir = snapshot.language === "arabic" ? "rtl" : "ltr";
      generated.lang = snapshot.language === "arabic" ? "ar" : "en";
  ...
  function updateAskAIOptionSelection(group, value, { emit = true } = {}) {
    if (group === "language") {
      askAIWorkspaceState.language = getOutputLanguage(value).value;
  ...
  function createAskAIOptionsCard() {
    ...
    card.append(
      createGroup("Output language", "language", OUTPUT_LANGUAGES),
  ...
  function renderAskAIOutputPanel() {
    ...
    summary.innerHTML = "<strong>Output:</strong> ";
    summary.append(`${getAskAIFocus().label} · ${getOutputLanguage(askAIWorkspaceState.language).label} · ${getAskAIResponse().label} · ${getAskAIKnowledgeMode().label}.`);
    scroller.append(header, summary);
    ...
    if (askAIWorkspaceState.answer) {
      ...
      answer.dir = askAIWorkspaceState.language === "arabic" ? "rtl" : "ltr";
      answer.lang = askAIWorkspaceState.language === "arabic" ? "ar" : "en";
  ...
  async function requestAskAIAnswer(source = "workspace", options = {}) {
    ...
    const lastRequest = {
      ...
      language: askAIWorkspaceState.language,
    ...
    document.dispatchEvent(new CustomEvent("sugo:askaigeneraterequest", {
      detail: { source, query, isFollowup, language: askAIWorkspaceState.language, response: askAIWorkspaceState.response, sop: askAIWorkspaceState.sop, focus: askAIWorkspaceState.focus }
    }));
  ...
  function applyAskAIAnswer(result = {}, { source = "api" } = {}) {
    ...
    if (Object.prototype.hasOwnProperty.call(payload, "language")) askAIWorkspaceState.language = getOutputLanguage(payload.language).value;
  ...
  function updateVisionOptionSelection(group, value, { emit = true } = {}) {
    ...
    if (note) {
      note.innerHTML = `<strong>Output:</strong> ${getVisionOption(VISION_OUTPUT_TYPES, visionWorkspaceState.output).label} · ${getVisionAnalysis().shortLabel} · ${getOutputLanguage(visionWorkspaceState.language).label} · ${getVisionOption(VISION_KNOWLEDGE_MODES, visionWorkspaceState.sop).label}.`;
    }
  ...
  function getVisionSnapshot() {
    return {
      output: visionWorkspaceState.output,
      language: visionWorkspaceState.language,
      response: visionWorkspaceState.response,
      sop: visionWorkspaceState.sop,
      analysis: visionWorkspaceState.analysis,
      userId: visionWorkspaceState.userId,
  ...
  function renderVisionSources(container) {
    ...
    const values = [
      ...
      ["Language", getOutputLanguage(visionWorkspaceState.language).label]
    ];
    values.forEach(([label, value]) => {
      const item = document.createElement("div");
      const strong = document.createElement("strong");
  ...
  function renderVisionOutputPanel() {
    ...
    if (visionWorkspaceState.answer) {
      ...
      answer.dir = visionWorkspaceState.language === "arabic" ? "rtl" : "ltr";
      answer.lang = visionWorkspaceState.language === "arabic" ? "ar" : "en";
      answer.innerHTML = window.SUGO?.WorkerAPI?.renderMarkdown
  ...
  async function requestVisionAnalysis(source = "workspace", options = {}) {
    ...
    document.dispatchEvent(new CustomEvent("sugo:visionanalysisrequest", {
      detail: { source, output: visionWorkspaceState.output, language: visionWorkspaceState.language, response: visionWorkspaceState.response, sop: visionWorkspaceState.sop, analysis: visionWorkspaceState.analysis }
    }));
  ...
  function retryVisionAnalysis() {
    ...
    visionWorkspaceState.language = getOutputLanguage(request.language).value;
  ...
  function applyVisionResult(result = {}, { source = "api" } = {}) {
    ...
    if (Object.prototype.hasOwnProperty.call(payload, "language")) visionWorkspaceState.language = getOutputLanguage(payload.language).value;
  ...
  function normalizeArticleSource(value) {
    return String(value || "")
      .replace(/\r\n?/g, "\n")
      .replace(/\u00a0/g, " ")
      .replace(/[ \t]+\n/g, "\n")
  ...
  function isArticleHeadingBlock(block, nextLine = "") {
    const line = String(block || "").trim();
    if (!line || line.includes("\n") || line.length > 96) return false;
    if (/^(?:https?:\/\/|www\.)/i.test(line)) return false;
    if (/^[•●▪◦‣⁃*+\-–—]|^[0-9٠-٩]+[.)\-:]/u.test(line)) return false;
    if (/[.!?؟؛;:]$/u.test(line)) return false;
    const words = line.split(/\s+/).filter(Boolean);
    if (words.length > 12) return false;
    const next = String(nextLine || "").trim();
    if (/^[•●▪◦‣⁃*+\-–—]|^[0-9٠-٩]+[.)\-:]/u.test(next)) return true;
  ...
  function appendArticleTextLines(container, text) {
    const lines = normalizeArticleSource(text).split("\n");
    let paragraph = null;
    let currentList = null;
    let currentListType = "";

    const closeParagraph = () => { paragraph = null; };
    const closeList = () => { currentList = null; currentListType = ""; };
    ...
    const ensureList = (type, start = 1) => {
      ...
      if (type === "ol" && start > 1) currentList.start = start;
    ...
    for (const rawLine of lines) {
      ...
      if (ordered || bullet) {
        const number = ordered ? Number(ordered[1].replace(/[٠-٩]/g, (digit) => "٠١٢٣٤٥٦٧٨٩".indexOf(digit))) || 1 : 1;
        const type = ordered ? "ol" : "ul";
        const list = ensureList(type, number);
  ...
  function createArticleLanguageContent(pane, field) {
    ...
    const normalized = normalizeArticleSource(field.text);
    ...
    if (lines.length && (lines[0].trim() === pane.title || (sourceTitle && lines[0].trim() === sourceTitle))) lines.shift();

    const groups = [];
    let current = { title: "", lines: [] };
    const flush = () => {
      if (current.title || current.lines.some((line) => line.trim())) groups.push(current);
      current = { title: "", lines: [] };
    };
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      if (isArticleHeadingBlock(line, next)) {
        flush();
        current.title = line;
      } else {
    ...
    if (!groups.length && normalized) groups.push({ title: "", lines: normalized.split("\n") });
    groups.forEach((group, index) => {
      const card = document.createElement(group.title ? "section" : "div");
      card.className = group.title ? "article-content-card" : "article-content-lead article-rich-text";
      ...
      if (group.title) {
        ...
        appendArticleTextLines(body, group.lines.join("\n"));
        card.append(title, body);
      } else {
        appendArticleTextLines(card, group.lines.join("\n"));
      }
      wrapper.append(card);
    });
